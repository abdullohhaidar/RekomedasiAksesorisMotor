...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    ...
    <div class="page-header">
        <h3 class="page-title"> Tabel Ban </h3>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @forelse($dataKatBan as $index => $ban)
                            <tr>
                                <td> {{ $index + 1 }} </td>
                                <td> {{ $ban->merk_ban }} </td>
                                <td> {{ $ban->tipe_ban }} </td>
                                <td>
                                    <a href="#" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#productModalEdit" onclick="showEditModal({{$ban->id_kategori_ban }})">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    
                                    <form action="{{ route('katBan.destroy', $ban->id_kategori_ban ) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="modal fade" id="productModalAdd" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content rounded-4 shadow-sm">
                ...
                <form id="productForm" action="{{ route('katBan.input') }}" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="modal fade" id="productModalEdit" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content rounded-4 shadow-sm">
                ...
                <div class="modal-body">
                <form id="productForm" action="{{ route('katBan.update', $ban->id_kategori_ban) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    
                        <!-- Setiap input diberi style tambahan -->
                        <div class="mb-3">
                            <input type="hidden" id="edit_id_ban" name="id_kategori_ban">
                        </div>
    ...
    function showEditModal(id) {
        ...
        fetch('/katBan/' + id)
            ...
            .then(data => {
                ...
                // form.action = baseRoute.replace('ID_PLACEHOLDER', data.id_ban);
                // document.getElementById('productForm').action = '/ban/update/' + data.id_ban;
                document.getElementById('edit_id_ban').value = data.id_kategori_ban;
                document.getElementById('edit_merk_ban').value = data.merk_ban;
                document.getElementById('edit_tipe_ban').value = data.tipe_ban;
                document.getElementById('edit_bug_ban').value = data.tipe_ban;
